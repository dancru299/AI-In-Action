...
def compare_models(prompt: str) -> dict:
    ...
    cost_estimate = (word_count / 0.75) / 1000 * COST_PER_1K_OUTPUT_TOKENS["gpt-4o"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    header = f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | {'GPT-4o Latency'} | {'Mini Latency'}"
    separator = "-" * len(header)
    lines = [header, separator]
    
    for r in results:
        p = r["prompt"].replace('\n', ' ')
        p = p[:37] + "..." if len(p) > 40 else p.ljust(40)
        
        gpt4o_r = r["gpt4o_response"].replace('\n', ' ')
        gpt4o_r = gpt4o_r[:37] + "..." if len(gpt4o_r) > 40 else gpt4o_r.ljust(40)
        
        mini_r = r["mini_response"].replace('\n', ' ')
        mini_r = mini_r[:37] + "..." if len(mini_r) > 40 else mini_r.ljust(40)
        
        gpt4o_l = f"{r['gpt4o_latency']:.2f}s".ljust(14)
        mini_l = f"{r['mini_latency']:.2f}s".ljust(12)
        
        line = f"{p} | {gpt4o_r} | {mini_r} | {gpt4o_l} | {mini_l}"
        lines.append(line)
